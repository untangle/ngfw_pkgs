...
#define HISTSIZE  100   /* histogramm */
  ...
  if (job->cfg.dust_size < 0 && job->res.numC > 0) { /* auto detection */
    ...
    /* maximum allowed dustsize */
    job->cfg.dust_size = (  ( job->res.sumX/job->res.numC )
                          * ( job->res.sumY/job->res.numC ) + 64) / 128;
    /* we assume that for rundum dust applies histo[i+1]<histo[i] */
    for (i=0;i+1<HISTSIZE && i<job->cfg.dust_size;i++){
     ...
     */
    if (vvv) fprintf(stderr,"# auto dust size = %d nC= %3d avD= %2d %2d\n",
                       job->cfg.dust_size, job->res.numC,
...
int remove_pictures( job_t *job){
  ...
  if (job->cfg.verbose)
    fprintf(stderr," pictures= %d  rest= %d  numC= %d\n# ...",j,j2,job->res.numC);
      ...
      if( box2->c==PICTURE
       && ( (box2->x0==0            || box2->y0==0)  /* on border? */
        ...
        if( j>8 ){ /* remove box if more than 8 chars are within box */
          ...
          free_box(box2); j=1; num_del++; break;
        } else j=0;
      }
    } end_for_each(&(job->res.boxlist));
  }
