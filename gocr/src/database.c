...
wchar_t ocr_db(struct box *box1) {
  ...
  if( (JOB->cfg.mode&128) != 0 && c == UNKNOWN ) for(;;) { // prompt user
    ...
    fprintf(stderr," The upper pattern was not recognized.\n"
    ...
    if (dd==1 && !(buf[0]&128)) {  c=buf[0]; utf8_ok=1; }
    if (dd>1 && dd<7) {
      int i, u0, u1; /* define UTF8-start sequences, u0=0bits u1=1bits */
      u0=       1<<(7-dd);
      u1=255&~((1<<(8-dd))-1);
    ...
    if (dd>0){
      if (utf8_ok==1) { setac(box1, c, 100); } /* store single char */
      if (utf8_ok==0) {    /* store a combination of chars (string) */
        c='_'; /* probably a bad idea? */
        setas(box1, buf, 100);
      }
      ...
      if (JOB->cfg.verbose)
        fprintf(stderr, "\n# got  char= %c  16bit= 0x%04x  string= \"%s\"\n",
