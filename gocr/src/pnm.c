...
void readpgm(char *name,pix *p,int vvv){	// raw-ppm
  char c1,c2,*pip;     int nx,ny,nc,mod,i,j;	// buffer
  ...
  //    TXT   P1    P2    P3
  //    RAW   P4    P5    P6
  if (c1!='P' || c2 <'1' || c2 >'6') F0("no PNM (P1..P6)"); // PNM-TXT/RAW
...
int writeppm(char *nam, pix *p){ /* P6 raw-ppm */
  ...
  for(x=0;x<p->x;x++){
    ...
    bits=(gray&128)>>4; bits|=bits>>1; bits|=bits>>2;
//    bits|=bits<<4;
    rgb[0]=rgb[1]=rgb[2]=(gray & (bits|0xF0)) | bits;
    if (gray<160) {  /* black */
      if(gray&2) { rgb[0]|=0x70; rgb[1]&=~0xF0; rgb[2]&=~0xF0; } /* blue */
      if(gray&4) { rgb[1]|=0x70; rgb[2]&=~0xF0; rgb[0]&=~0xF0; }
      if(gray&8) { rgb[2]|=0x70; rgb[0]&=~0xF0; rgb[1]&=~0xF0; } /* red */
      if(gray&1) { rgb[2]|=0x70; rgb[0]|= 0x70; rgb[1]&=~0xF0; } /* cyan */
    } else {         /* white (box frame) */
      if(gray&2) { rgb[0]|= 0xF0; rgb[1]&=~0x80; rgb[2]&=~0x80; } /* red */
      if(gray&4) { rgb[1]|= 0xF0; rgb[2]&=~0x30; rgb[0]&=~0x30; } /* green */
      if(gray&8) { rgb[2]|= 0xF0; rgb[0]&=~0x20; rgb[1]&=~0x20; } /* blue */
      if(gray&1) { rgb[2]|= 0xF0; rgb[0]|= 0xF0; rgb[1]&=~0xC0; } /* cyan */
    }
