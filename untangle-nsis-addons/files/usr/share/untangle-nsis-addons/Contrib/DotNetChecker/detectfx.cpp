...
/******************************************************************
Function Name:	IsNetfx45FullInstalled
Description:	Uses the detection method recommended at
                http://msdn.microsoft.com/library/ee942965(v=VS.100).aspx
                to determine whether the .NET Framework 4 Full is
                installed on the machine
...
{
	EXDLL_INIT();
	pushstring((IsNetfx45Installed() && CheckNetfxVersionUsingMscoree(g_szNetfx45VersionString)) ? L"true" : L"false");
}
...
void __declspec(dllexport) GetDotNet45ServicePack(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop, extra_parameters *extra) 
{
	...
	bool bNetfx45Installed = (IsNetfx45Installed() && CheckNetfxVersionUsingMscoree(g_szNetfx45VersionString));
	TCHAR szMessage[MAX_PATH];
	TCHAR szOutputString[MAX_PATH*20];

	if (bNetfx45Installed)
	{
		iNetfx45SPLevel = GetNetfxSPLevel(g_szNetfx45RegKeyName, g_szNetfx45SPxRegValueName);
