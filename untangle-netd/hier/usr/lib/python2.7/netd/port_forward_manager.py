...
class PortForwardManager:
    ...
    def write_admin_port_rules( self, settings, prefix="", verbosity=0):
        ...
        self.file.write("${IPTABLES} -t nat -A port-forward-rules -p tcp -m addrtype --dst-type local --destination-port 443 -j DNAT --to-destination ${ADDR}:0 -m comment --comment \"Drop local HTTPS traffic that hasn't been handled earlier in chain\"" + "\n")
        self.file.write("\n");
        ...
        self.file.write("${IPTABLES} -t nat -A port-forward-rules -p tcp -m addrtype --dst-type local --destination-port 80 -j DNAT --to-destination ${ADDR}:0 -m comment --comment \"Drop local HTTP traffic that hasn't been handled earlier in chain\"" + "\n")
        self.file.write("\n");
            ...
            if intf.get('configType') == 'ADDRESSED' and intf.get('isWan'):
                ...
                for sub_intf in settings.get('interfaces').get('list'):
                    if sub_intf.get('configType') == 'BRIDGED' and sub_intf.get('bridgedTo') == intf.get('interfaceId'):
                        ...
                        self.file.write("\t${IPTABLES} -t nat -A port-forward-rules -p tcp -m mark --mark 0x%04X/0x%04X --destination ${ADDR} --destination-port 80 -j DNAT --to-destination ${ADDR}:0 -m comment --comment \"Drop local HTTP traffic that hasn't been handled earlier in chain\"" % (sub_intf.get('interfaceId'), self.srcInterfaceMarkMask) + "\n")
                        self.file.write("fi" + "\n")
                        self.file.write("\n");
