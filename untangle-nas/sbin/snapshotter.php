 ...
 */
	...
	/* First take a lock so that this program is an exclusive process and no other
	   nas code such as the web-interface is also operating at the same time
	   trying to snapshot stuff.
	*/

	$flockp = fopen("/opt/nas/var/oflocks/snap.lock", "w+");
	...
	$snapshotsp = popen("/usr/bin/sudo /usr/bin/tee /opt/nas/etc/snapshots.xml", "w");
	...
	/*	$service_winbind = (strstr(exec("/usr/bin/sudo /sbin/chkconfig --list winbind"), "3:on") ? 1 : 0);
		$service_smb = (strstr(exec("/usr/bin/sudo /sbin/chkconfig --list samba"), "3:on") ? 1 : 0);
		$service_nfsv3 = (strstr(exec("/usr/bin/sudo /sbin/chkconfig --list nfs-kernel-server"), "3:on") ? 1 : 0);
		$service_http = (strstr(exec("/usr/bin/sudo /sbin/chkconfig --list httpd"), "3:on") ? 1 : 0);
        	$service_iscsi = (strstr(exec("/usr/bin/sudo /sbin/chkconfig --list iscsitarget"), "3:on") ? 1 : 0);
        	$service_ftp = (strstr(exec("/usr/bin/sudo /sbin/chkconfig --list proftpd"), "3:on") ? 1 : 0);
	*/

		$service_winbind = (strstr(exec("/opt/nas/sbin/service-status winbind"), "running") ? 1 : 0);
		$service_smb = (strstr(exec("/opt/nas/sbin/service-status samba"), "running") ? 1 : 0);
		$service_nfsv3 = (strstr(exec("/opt/nas/sbin/service-status nfs-kernel-server"), "running") ? 1 : 0);
		$service_http = (strstr(exec("/opt/nas/sbin/service-status httpd"), "running") ? 1 : 0);
		$service_ftp = (strstr(exec("/opt/nas/sbin/service-status proftpd"), "running") ? 1 : 0);
		$service_iscsi = (strstr(exec("/opt/nas/sbin/service-status iscsitarget"), "running") ? 1 : 0);
                $service_rsync = (strstr(exec("/opt/nas/sbin/service-status rsync"), "running") ? 1 : 0); 
