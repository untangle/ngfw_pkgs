...
foreach $d (@devs) {
    ...
    if ($d =~ m/\/dev\/(loop\d*)/) {
	# Handle loop specially since we can't use parted
	$disk = { 'device' => $d,
		  'prefix' => $d };
	open(LOOPSIZE, "/sys/block/$1/size");
	local $loopsize = <LOOPSIZE>;
	chomp $loopsize;
	$disk->{'sectors'} = $loopsize;
	close(LOOPSIZE);
	open(LOOPSTAT, "losetup $d |");
	$disk->{'model'} = <LOOPSTAT>;
	chomp $disk->{'model'};
	$disk->{'model'} =~ s/.*?\((.+?)\).*/$1/;
	close(LOOPSTAT);
	...
	$part->{'desc'} =
	    &text('select_part', 'LOOP', "$1", "$2");
	push(@{$disk->{'parts'}}, $part);
	next;
    }
	

	open(PARTED, "LANG=C parted $d print 2>/dev/null |");
	...
	while($str = <PARTED>)
	{
		...
		{
			system("parted $d mklabel gpt 2>/dev/null");
			$disk_label = "gpt";
			break;
		}
	}
