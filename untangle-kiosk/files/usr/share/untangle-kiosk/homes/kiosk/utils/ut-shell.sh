...
if sudo grep -qE '^root:(\*|YKN4WuGxhHpIw|$1$3kRMklXp$W/hDwKvL8GFi5Vdo3jtKC\.|CHANGEME):' /etc/shadow ; then
  ...
  echo "You will need to enter the same password twice to set the new password." 
  echo 
  echo "It will not echo your keystrokes to the screen for security."
  echo "Please choose a password that is not easily guessed, especially if you"
  echo "have, or plan on enabling ssh access to the system."
  echo 
