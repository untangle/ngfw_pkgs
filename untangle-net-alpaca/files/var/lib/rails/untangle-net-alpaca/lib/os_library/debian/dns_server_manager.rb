...
EOF
  ...
  def dhcp_config( dhcp_server_settings, dns_server_settings )
    ...
    if ( IPAddr.parse( dhcp_server_settings.start ).nil? || 
         IPAddr.parse( dhcp_server_settings.end ).nil? )
    ...
    ## Setup the range
    settings << "#{FlagRange}=#{dhcp_server_settings.start},#{dhcp_server_settings.end},#{duration}"
  ...
  def calculate_gateway( dhcp_server_settings )
    ...
    return nil if IPAddr.parse( dhcp_server_settings.start ).nil?
    ...
    route = `ip route get #{dhcp_server_settings.start}`.split( "\n" )[0]
