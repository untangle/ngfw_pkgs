...
if [ -d $PG_VAR_DIR_OLD ] ; then
  ...
  pushd /tmp
  su postgres -c "/usr/lib/postgresql/9.4/bin/pg_upgrade --link -b $PG_BIN_DIR_OLD -B $PG_BIN_DIR_NEW -d $PG_VAR_DIR_OLD -D $PG_VAR_DIR_NEW -o ' -c config_file='$PG_CONF_OLD -O ' -c config_file='$PG_CONF_NEW"
