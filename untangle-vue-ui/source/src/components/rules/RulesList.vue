...
<script>
  ...
  import { RulesList, ruleDefsNgfw } from 'vuntangle'
    ...
    provide() {
      return {
        ...
        $features: {
          ...
          hasIpv6Support: ['filter'].includes(this.ruleType),
        },
        $readOnly: false,
      }
    },
    ...
    computed: {
      ...
      interfaces: ({ ruleType }) => {
        ...
        switch (ruleType) {
          case 'port-forward':
          case 'nat':
            interfaces = util.getInterfaceList(true, true)
