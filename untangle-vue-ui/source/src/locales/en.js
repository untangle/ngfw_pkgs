export default {
  A_wizard_will_guide: 'A wizard will guide you through the initial setup and configuration of the',
  Run_Setup_Wizard: 'Run Setup Wizard',
  Thanks_for_choosing: 'Thanks for choosing',
  To_continue_you_must_connect_to_ETM_Dashboard_Offline:
    'To continue, you must connect to ETM Dashboard, which is currently unreachable from this device. Configure Internet connectivity to proceed.',
  To_continue_you_must_log_account_online:
    'To continue, you must log in using your ETM Dashboard account. If you do not have one, you can create a free account.',
  about: 'About',
  any: 'Any',
  any_non_wan: 'Any Non-WAN',
  any_wan: 'Any WAN',
  client_tagged: 'Client Tagged',
  dst_addr: 'Destination Address',
  dst_intf: 'Destination Interface',
  dst_local: 'Destined Local',
  dst_port: 'Destination Port',
  ipsec_vpn: 'IPsec VPN',
  ipv6_support_enabled: 'IPv6 Support Enabled',
  port_forward_description:
    "Port Forward rules forward sessions matching the configured criteria from a public IP to an IP on an internal (NAT'd) network. The rules are evaluated in order.",
  server_tagged: 'Server Tagged',
  src_addr: 'Source Address',
  src_intf: 'Source Interface',
  src_mac: 'Client Mac Address',
}
