/*
 * $HeadURL: svn://chef/work/src/libnetcap/src/barfight_trie.c $
...
static barfight_trie_element_t _create_element( barfight_trie_t* trie, barfight_trie_level_t* parent, 
                                              struct in_addr* ip, u_char pos );

static int _set_line( barfight_trie_line_t* line, barfight_trie_base_t* base );

/* Decrements num_children for all of parent, and all of its parents */
static int _decrement_children( barfight_trie_t* trie, barfight_trie_level_t* parent, struct in_addr* _ip );

/* Get the closest item to ip, this never creates a new item  */
int barfight_trie_get            ( barfight_trie_t* trie, struct in_addr* _ip, barfight_trie_line_t* line )
{
    if ( trie == NULL || _ip == NULL || line == NULL ) return errlogargs();
    
    barfight_trie_level_t* parent = &trie->root;
    barfight_trie_level_t* child;
    int c;
    ...
    debug( NC_TRIE_DEBUG_HIGH, "barfight_trie_get: %#010x", _ip->s_addr ); 
...
int barfight_trie_insert_and_get ( barfight_trie_t* trie, struct in_addr* _ip, pthread_mutex_t* mutex,
                                     barfight_trie_line_t* line )
{
    ...
    int _critical_section( void ) {
        ...
        if (( item = ht_lookup( &trie->ip_element_table, (void*)(long)_ip->s_addr )) != NULL ) {
            if ( _set_line( line, (barfight_trie_base_t*)item ) < 0 ) {
        ...
        debug( NC_TRIE_DEBUG_HIGH, "barfight_trie_insert_and_get: %#010x", _ip->s_addr );
    ...
    if (( item = ht_lookup( &trie->ip_element_table, (void*)(long)_ip->s_addr )) != NULL ) {
        if ( _set_line( line, (barfight_trie_base_t*)item ) < 0 ) return errlog( ERR_CRITICAL, "_set_line\n" );
        return 0;
    }
 ...
 */
int barfight_trie_remove         ( barfight_trie_t* trie, struct in_addr* _ip, pthread_mutex_t* mutex,
                                     barfight_trie_line_t* line )
{
    if ( trie == NULL || _ip == NULL || line == NULL ) return errlogargs();
    ...
    debug( NC_TRIE_DEBUG_HIGH, "barfight_trie_remove: %#010x", _ip->s_addr );
...
int barfight_trie_line_destroy   ( barfight_trie_t* trie, barfight_trie_line_t* line )
{
    ...
    for ( c = 0 ; c < count ; c++ ) {
        ...
        barfight_trie_element_raze( trie, line->d[c] );
        line->d[c].item = NULL;
    }
...
int barfight_trie_line_raze      ( barfight_trie_t* trie, barfight_trie_line_t* line )
{
    ...
    if ( barfight_trie_line_destroy( trie, line ) < 0 ) {
        errlog( ERR_CRITICAL, "barfight_trie_line_destroy\n" );
    }

    if ( barfight_trie_line_free( line ) < 0 ) {
...
/* This copies the item itself, not a barfight_trie_item */
int barfight_trie_init_data          ( barfight_trie_t* trie, barfight_trie_item_t* dest, void* src, 
                                              ...
                                              struct in_addr* ip, u_char pos )
{
    ...
    if ( NC_TRIE_DEPTH_TOTAL == depth ) {
        if (( child.item = barfight_trie_item_create( trie, parent, pos, depth )) == NULL ) {
            debug_nodate ( NC_TRIE_DEBUG_HIGH,"\n" );
            errlog( ERR_CRITICAL, "barfight_trie_item_create\n" );
            return child;
        }
    } else {
        if (( child.level = barfight_trie_level_create( trie, parent, pos, depth )) == NULL ) {
            debug_nodate ( NC_TRIE_DEBUG_HIGH,"\n" );
            errlog( ERR_CRITICAL, "barfight_trie_level_create\n" );
            return child;
        }
    }
    ...
    if ( barfight_trie_init_data( trie, child.item, parent->base.data, ip, depth ) < 0 ) {
        barfight_trie_element_raze( trie, child );
        errlog( ERR_CRITICAL, "barfight_trie_init_data\n" );
        child.base = NULL;
        return child;
    }
...
static int _set_line( barfight_trie_line_t* line, barfight_trie_base_t* base )
{
    ...
    for (  ; c >= 0 && ( base != NULL ) ; c-- ) {
        ...
        base = (barfight_trie_base_t*)base->parent;
    }
