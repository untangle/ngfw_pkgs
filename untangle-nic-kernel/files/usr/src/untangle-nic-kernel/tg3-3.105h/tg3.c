...
#define DRV_MODULE_VERSION	"3.105h"
#define DRV_MODULE_RELDATE	"February 4, 2010"
...
#define TG3_XFER_STD_OFLOW		0x40000000
#define TG3_XFER_JMB_OFLOW		0x80000000
...
static void tg3_phy_toggle_apd(struct tg3 *tp, bool enable)
{
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5785) {
		reg = MII_TG3_MISC_SHDW_WREN |
		      ...
		      MII_TG3_MISC_SHDW_SCR5_TRDDAPD |
		      MII_TG3_MISC_SHDW_SCR5_LPED |
		      MII_TG3_MISC_SHDW_SCR5_DLPTLM |
		      MII_TG3_MISC_SHDW_SCR5_SDTL;
		if ((tp->phy_id & ~PHY_ID_MASK) < 0x3) {
...
out:
	...
	if (tp->tg3_flags & TG3_FLAG_JUMBO_CAPABLE) {
		...
		/* Set Extended packet length bit on
		 * all chips that support jumbo frames.
		 */
		if ((tp->phy_id & PHY_ID_MASK) == PHY_ID_BCM5401)
			/* Cannot do read-modify-write on 5401 */
			tg3_writephy(tp, MII_TG3_AUX_CTRL, 0x4c20);
		else if (!tg3_writephy(tp, MII_TG3_AUX_CTRL,
				       MII_TG3_AUXCTL_SHDWSEL_MISC) &&
			 !tg3_readphy(tp, MII_TG3_AUX_CTRL, &phy_val))
			tg3_writephy(tp, MII_TG3_AUX_CTRL,
			             phy_val | MII_TG3_AUXCTL_RX_EXT_PKT_LEN);

		/* Set high fifo elasticity to support
		 * jumbo frames transmission.
		 */
		if (!tg3_readphy(tp, MII_TG3_EXT_CTRL, &phy_val))
		    tg3_writephy(tp, MII_TG3_EXT_CTRL,
				 phy_val | MII_TG3_EXT_CTRL_FIFO_ELASTIC);
	}
...
static int tg3_rxing_cfg_code_words(struct tg3 *tp)
{
	u32 val;
	int ret = -EIO;
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5717) {
		/* 1000XStatus2 */
		if (!tg3_writephy(tp, 0x1f, 0x0) &&
		    !tg3_readphy(tp, 0x15, &val) &&
		    !tg3_readphy(tp, 0x15, &val))
			ret = val & 0x0200;
	} else {
		/* Select expansion interrupt status register */
		if (!tg3_writephy(tp, 0x17, 0x0f01) &&
		    !tg3_readphy(tp, 0x15, &val) &&
		    !tg3_readphy(tp, 0x15, &val))
			ret = val & 0x20;
	}
...
static int tg3_signal_detected(struct tg3 *tp)
{
	u32 val;
	int ret = -EIO;
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5717) {
		if (!tg3_writephy(tp, 0x1f, 0x0) &&
		    !tg3_readphy(tp, 0x16, &val))
			ret = val & 0x40;
	} else {
		/* Select shadow register 0x1f */
		if (!tg3_writephy(tp, 0x1c, 0x7c00) &&
		    !tg3_readphy(tp, 0x1c, &val))
			ret = val & 0x10;
	}
...
static void tg3_serdes_parallel_detect(struct tg3 *tp)
{
	...
	if (!netif_carrier_ok(tp->dev)) {
		...
		tg3_readphy(tp, MII_BMCR, &bmcr);
		if ((bmcr & BMCR_ANENABLE) &&
		    tg3_signal_detected(tp) > 0 &&
		    !tg3_rxing_cfg_code_words(tp)) {
			/* Link is up by parallel detection. */
			bmcr &= ~BMCR_ANENABLE;
	...
	while (sw_idx != hw_idx && budget > 0) {
		...
		if (len > TG3_RX_COPY_THRESH(tp)) {
			...
			copy_skb = netdev_alloc_skb(tp->dev,
						    len + TG3_RAW_IP_ALIGN + VLAN_HLEN);
			if (copy_skb == NULL)
...
next_pkt_nopost:
	...
	if (!(tp->tg3_flags3 & TG3_FLG3_ENABLE_RSS)) {
		...
		tg3_netif_rx_schedule(tp->dev, &tp->napi[0].napi);
	}
...
static u32 tg3_rx_prodring_xfer(struct tg3 *tp,
				struct tg3_rx_prodring_set *dpr,
				struct tg3_rx_prodring_set *spr, u32 stat_bit)
{
	u32 si, di, cpycnt, src_prod_idx, status = 0;
	int i;
...
static int tg3_poll_work(struct tg3_napi *tnapi, int work_done, int budget)
{
	...
	if ((tp->tg3_flags3 & TG3_FLG3_ENABLE_RSS) && tnapi == &tp->napi[0]) {
		int i;
		u32 stat = 0;
		...
		for (i = 1; i < tp->irq_cnt; i++) {
			struct tg3_napi *snapi = &tp->napi[i];

			stat |= tg3_rx_prodring_xfer(tp, tnapi->prodring,
						     snapi->prodring, snapi->coal_now);
		}

		wmb();

		if (stat & (TG3_XFER_STD_OFLOW | TG3_XFER_JMB_OFLOW)) {
			u32 coal = 0;

			/* Create a list of vectors that did
			 * not update the producer rings.
			 */
			if (stat & TG3_XFER_STD_OFLOW)
				coal |= ~stat;

			if (stat & TG3_XFER_JMB_OFLOW)
				coal |= ~stat << 4;

			tw32_f(HOSTCC_MODE, tp->coalesce_mode |
			       HOSTCC_MODE_ENABLE |
			       (coal & HOSTCC_MODE_COAL_NOW_RSS_MASK));
		}
...
static int tg3_poll_msix(struct napi_struct *napi, int budget)
{
	...
	while (1) {
		...
		if (likely(sblk->idx[0].tx_consumer == tnapi->tx_cons &&
			   *(tnapi->rx_rcb_prod_idx) == tnapi->rx_rcb_ptr)) {
			tg3_netif_rx_complete(tp->dev, napi);
			/* Reenable interrupts. */
			tw32_mailbox(tnapi->int_mbox, tnapi->last_tag << 24);
			mmiowb();
			break;
...
#endif
{
	...
	if (likely(!tg3_irq_sync(tp)))
		tg3_netif_rx_schedule(tp->dev, &tnapi->napi);
				       ...
				       u32 *start, u32 base_flags, u32 mss)
{
	...
	if (!new_skb) {
		...
		if (tg3_pci_dma_mapping_error(tp->pdev, new_addr)) {
			ret = -1;
...
static int tg3_poll_fw(struct tg3 *tp)
{
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_57765) {
		/* Wait for an additional 100 milliseconds for the 57765
		 * needs a little more time to do some important work.
		 */
		for (i = 0; i < 1000; i++)
			udelay(10);
	}
...
static int tg3_request_firmware(struct tg3 *tp)
{
	...
	tp->fw_len = fw_data[2];	/* includes bss */
...
static int tg3_run_loopback(struct tg3 *tp, int loopback_mode)
{
	...
	if (tg3_pci_dma_mapping_error(tp->pdev, map)) {
		dev_kfree_skb(skb);
		return -EIO;
	}
...
#define TG3_VPD_LRDT			0x80	/* Large Resource Data Type */
#define TG3_VPD_LRDT_ID(x)		(x | TG3_VPD_LRDT)

/* Large Resource Data Type Item Names */
#define TG3_VPD_LRDT_LIN_ID_STRING	0x02	/* Identifier String */
#define TG3_VPD_LRDT_LIN_RO_DATA	0x10	/* Read-Only Data */
#define TG3_VPD_LRDT_LIN_RW_DATA	0x11	/* Read-Write Data */

#define TG3_VPD_LRDT_ID_STRING		TG3_VPD_LRDT_ID(TG3_VPD_LRDT_LIN_ID_STRING)
#define TG3_VPD_LRDT_RO_DATA		TG3_VPD_LRDT_ID(TG3_VPD_LRDT_LIN_RO_DATA)
#define TG3_VPD_LRDT_RW_DATA		TG3_VPD_LRDT_ID(TG3_VPD_LRDT_LIN_RW_DATA)

/* Small Resource Data Type Item Names */
#define TG3_VPD_SRDT_SIN_END		0x78	/* End */

#define TG3_VPD_SRDT_END		TG3_VPD_SRDT_SIN_END

#define TG3_VPD_RO_KEYWORD_PARTNO	"PN"
#define TG3_VPD_RO_KEYWORD_VENDOR0	"V0"

#define TG3_VPD_SRDT_SIN_MASK		0x78
#define TG3_VPD_SRDT_LEN_MASK		0x07

#define TG3_VPD_LRDT_TAG_SIZE		3
#define TG3_VPD_SRDT_TAG_SIZE		1
#define TG3_VPD_INFO_FLD_HDR_SIZE	3

static inline u16 tg3_vpd_lrdt_size(u8 *lrdt)
{
	return (u16)lrdt[1] + ((u16)lrdt[2] << 8);
}

static inline u8 tg3_vpd_srdt_size(u8 *srdt)
{
	return (*srdt) & TG3_VPD_SRDT_LEN_MASK;
}

static int __devinit tg3_vpd_find_tag(u8 *data, unsigned int start,
				      unsigned int len, u8 tagid)
{
	...
	for (i = start; i < len; ) {
		...
		if (val & TG3_VPD_LRDT) {
			...
			if (i + TG3_VPD_LRDT_TAG_SIZE > len)
				break;

			if (val == tagid)
				return i;

			i += TG3_VPD_LRDT_TAG_SIZE +
			     tg3_vpd_lrdt_size(&data[i]);
		} else {
			u8 tag = val & ~TG3_VPD_SRDT_LEN_MASK;

			if (tag == tagid)
				return i;

			if (tag == TG3_VPD_SRDT_SIN_END)
				break;

			i += TG3_VPD_SRDT_TAG_SIZE +
			     tg3_vpd_srdt_size(&data[i]);
		}
	}
...
static inline u8 tg3_vpd_info_field_size(u8 *info_field)
{
	return info_field[2];
}

static int __devinit tg3_vpd_rodata_find_if_keyword(u8 *rodata,
						    unsigned int start,
						    unsigned int rosize,
						    char *kw)
{
	...
	for (i = start; i + TG3_VPD_INFO_FLD_HDR_SIZE <= start + rosize;) {
		if (rodata[i + 0] == kw[0] &&
		    rodata[i + 1] == kw[1])
			return i;

		i += TG3_VPD_INFO_FLD_HDR_SIZE +
		     tg3_vpd_info_field_size(&rodata[i]);
	}
...
{
	u8 vpd_data[TG3_NVM_VPD_LEN];   /* in little-endian format */
	int i;
	...
	if (magic == TG3_EEPROM_MAGIC) {
		...
		ssize_t cnt;
		cnt = pci_read_vpd(tp->pdev, 0, TG3_NVM_VPD_LEN, vpd_data);
		if (cnt < TG3_NVM_VPD_LEN)
			goto out_not_found;
	}

	i = tg3_vpd_find_tag(vpd_data, 0, TG3_NVM_VPD_LEN,
			     TG3_VPD_LRDT_RO_DATA);
	if (i < 0)
		goto out_not_found;

	block_end = i + TG3_VPD_LRDT_TAG_SIZE +
		    tg3_vpd_lrdt_size(&vpd_data[i]);
	...
	if (tp->pdev->subsystem_vendor == PCI_VENDOR_ID_DELL) {
		...
		rodi = tg3_vpd_rodata_find_if_keyword(vpd_data, i, block_end,
						   TG3_VPD_RO_KEYWORD_VENDOR0);
		if (rodi >= 0) {
			u8 len;

			len = tg3_vpd_info_field_size(&vpd_data[rodi]);

			rodi += TG3_VPD_INFO_FLD_HDR_SIZE;
			if (len + rodi <= TG3_NVM_VPD_LEN) {
				memcpy(&tp->fw_ver[0], &vpd_data[rodi],
				       min_t(u8, TG3_VER_SIZE - 6, len));
				strcat(tp->fw_ver, " BC: ");
			}
		}
	}

	i = tg3_vpd_rodata_find_if_keyword(vpd_data, i, block_end,
					   TG3_VPD_RO_KEYWORD_PARTNO);
	if (i >= 0) {
		u8 len;

		len = tg3_vpd_info_field_size(&vpd_data[i]);
...
static int __devinit tg3_get_invariants(struct tg3 *tp)
{
	...
	if (tp->pci_chip_rev_id == CHIPREV_ID_5700_B0)
		...
		if (tp->tg3_flags3 & TG3_FLG3_5755_PLUS)
			tp->dev->features |= NETIF_F_HW_CSUM;
		else
...
#else
	target_state = pci_choose_state(pdev, state);
#endif
